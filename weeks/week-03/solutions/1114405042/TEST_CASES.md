...
## 5. UVA 100 - The 3n + 1 problem (`test_100.py`)

### 5.1 範例測資 (Sample Input)
* **目的**：驗證 Collatz 猜想的 cycle-length 計算，以及在給定區間內找出最大值的邏輯是否正確。
* **輸入**：
  ```
  1 10
  100 200
...
### 5.2 邊界測資 (Edge Case) - i > j 反向輸入
* **目的**：驗證當題目給定的起點大於終點時 (例如 `i=10`, `j=1`)，程式能夠正確處理反向區間，**且輸出時仍然維持原本的 `i j max_cycle` 順序**。
* **輸入**：`10 1`
* **預期輸出**：`10 1 20`
